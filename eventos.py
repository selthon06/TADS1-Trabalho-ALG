...
def listar_eventos():
    print('\n--- Lista de Eventos ---')
    if not dados_eventos.eventos:
        print("Nenhum evento cadastrado.")
        return

    for idx, evento in enumerate(dados_eventos.eventos, start=1):
...
def adicionar_participante_evento():
    ...
    evento_encontrado = next((e for e in dados_eventos.eventos if e["nome"].lower() == nome_evento.lower()), None)
    if not evento_encontrado:
        print("Evento não encontrado.")
        return

    participante = next((p for p in dados_participantes.participantes if p["codigo"] == codigo_part), None)
    if not participante:
    ...
    for evento in dados_eventos.eventos:
        if evento["nome"].lower() == nome_evento.lower():
            ...
            for codigo in evento["participantes"]:
                participante = next((p for p in dados_participantes.participantes if p["codigo"] == codigo), None)
                if participante:
...
def filtrar_eventos_por_tema():
    ...
    encontrados = [evento for evento in dados_eventos.eventos if evento['tema'].strip().lower() == tema_busca]
